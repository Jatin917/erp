  ...
  return (
    <aside className={`${sidebarOpen ? "translate-x-0" : "-translate-x-full"} fixed z-40 h-full w-72 bg-white/90 dark:bg-slate-900/90 backdrop-blur border-r border-slate-200 dark:border-slate-800 p-3 transition-transform ease-in-out md:static md:translate-x-0`}>
      <div className="flex items-center justify-between mb-2">
        ...
        <button onClick={() => setSidebar(false)} className="md:hidden p-2 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800">
          <X className="h-5 w-5" />
        </button>
      </div>
      ...
      <nav className="mt-2 space-y-1">
        {items.map((n) => {
          ...
          const Icon = n.icon;
          return (
            <Link key={n.to} to={n.to} className={`group flex items-center gap-3 px-3 py-2 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800 ${active ? "bg-slate-100 dark:bg-slate-800" : ""}`}>
      ...
      <PermissionGuard allowPermissions={["VIEW_FEE_SUMMARY"]}>
        <div className="mt-6 p-3 rounded-2xl bg-gradient-to-br from-slate-100 to-slate-50 dark:from-slate-800 dark:to-slate-900 border border-slate-200 dark:border-slate-800">
          <div className="text-xs uppercase tracking-wider opacity-60">Quick Stats</div>
          <div className="mt-2 grid grid-cols-2 gap-2 text-sm">
            <div className="p-2 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800">
              <div className="opacity-60">Students</div>
              <div className="text-lg font-semibold">1,284</div>
            </div>
            <div className="p-2 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800">
              <div className="opacity-60">Due (₹)</div>
              <div className="text-lg font-semibold">3.2L</div>
            </div>
          </div>
        </div>
      </PermissionGuard>
