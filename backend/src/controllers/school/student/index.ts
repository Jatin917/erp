...
// Generate Barcode
// ----------------------
async function generateBarcode(student: Student) {
  // Prepare the QR text
...
function normalizeSession(session: string): string {
  ...
  if (parts.length !== 2) return session; // fallback if invalid format
...
export const createStudent = async (req: any, res: any) => {
  try {
    ...
    if (!branchId || !resolvedClassName) {
      return sendError(
        res,
        "branchId and className required",
    ...
    const student = await prisma.$transaction(async (tx) => {
      ...
      const parsedCustomFields =
        typeof data.customFields === "string" && data.customFields.trim().length
...
export const fetchStudents = async (req: Request, res: Response) => {
  try {
    ...
    if (session) {
      enrollmentWhere.session = { name: session as string };
    }
    if (className || section) {
      ...
      if (section) {
        enrollmentWhere.class.section = { id: section as string };
      }
    }
    if (rollNo) {
...
export const getStudentDetail = async (req: any, res: any) => {
  try {
    const { id } = req.params;
    ...
    const sectionId =
      latestEnrollment?.class?.sectionId ??
...
export const updateStudent = async (req: any, res: any) => {
  try {
    const { id } = req.params;
    const { className, class: classFromFrontend, branchId, rollNo, dob, ...data } =
      req.body;
    const photo = req.file;
    const resolvedClassName = className || classFromFrontend;
    ...
    let photoUrl: string | null = student.photoUrl;
    if (photo) {
      try {
        const uploadDir = getStudentDir(
          student.branch.schoolId,
...
export const downloadSampleSheetForBulkUpload = (req: any, res: any) => {
  try {
    console.log("yha request aa rhi hain");
    const filePath = path.join(
      __dirname,
      "../../../../uploads/sample-sheets/student_data_sample.xlsx"
    );
