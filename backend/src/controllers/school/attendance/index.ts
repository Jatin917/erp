 ...
 */
function doTimeSlotsOverlap(start1: string, end1: string, start2: string, end2: string): boolean {
...
export const upsertLectureFromDate = async (req: Request, res: Response) => {
  try {
    const {
      classId,
      branchId,
      weekDay,
      lecture, // { subjectId, teacherId, startTime, endTime }
      applyDate,
    } = req.body;
    ...
    await prisma.$transaction(async (tx) => {
      for (const day of schoolDays) {
        ...
        const teacherTimeConflict = teacherConflicts.find((conflict) =>
          doTimeSlotsOverlap(
            conflict.startTime,
            conflict.endTime,
            lecture.startTime,
        ...
        if (teacherTimeConflict) {
          throw new Error(
            `Teacher conflict: Teacher is already scheduled for an overlapping time slot (${teacherTimeConflict.startTime} - ${teacherTimeConflict.endTime}) on ${day.date.toISOString().split('T')[0]}`
          );
        }
        ...
        const classTimeConflict = classConflicts.find((conflict) =>
          doTimeSlotsOverlap(
            conflict.startTime,
            conflict.endTime,
            lecture.startTime,
        ...
        if (classTimeConflict) {
          throw new Error(
            `Class conflict: Class already has a scheduled lecture for an overlapping time slot (${classTimeConflict.startTime} - ${classTimeConflict.endTime}) on ${day.date.toISOString().split('T')[0]}`
          );
        }
    ...
    return sendSuccess(
      res,
      "Lecture(s) scheduled successfully",
      {
        createdCount,
        updatedCount,
        cancelledCount,
