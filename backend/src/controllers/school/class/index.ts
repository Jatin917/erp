...
export const getAllClass = async (req: any, res: any) => {
  try {
    ...
    const grouped: Record<string, { classId:string, name: string; sections: { id: string; name: string }[] }> = {};
...
export const createOrUpdateClass = async (req: any, res: any) => {
  ...
  try {
    ...
    await prisma.$transaction(async (tx) => {
      ...
      // 4️⃣ Calculate diffs
      const sectionsToAdd = sectionIds.filter((id: string) => !existingSectionIds.includes(id));
      const sectionsToRemove = existingClasses.filter(
        (cls) => cls.sectionId && !sectionIds.includes(cls.sectionId)
      );
    ...
    const grouped: Record<
      string,
      { name: string; sections: { id: string; name: string }[] }
    > = {};
...
export const getClassNames = async (req: any, res: any) => {
  try {
...
export const createClassName = async (req: any, res: any) => {
  try {
    const { name, branchId } = req.body;
...
export const createSubject = async (req: any, res: any) => {
  try {
    ...
    if (existing) {
      return sendError(
        res,
        `Subject "${trimmedName}" already exists for this class`,
...
export const getSubjects = async (req: any, res: any) => {
  try {
    ...
    const where:any = {class:{branchId}};
    if(classId){
...
export const deleteSubject = async (req: any, res: any) => {
  try {
    ...
    if (subject.lectures.length > 0) {
      return sendError(
        res,
        "Cannot delete subject that has lectures assigned",
        HTTP_STATUS.CONFLICT,
      );
    }
...
export const getFaculty = async (req:any, res:any) =>{
  try {
