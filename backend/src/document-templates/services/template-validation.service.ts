...
export class TemplateValidationService {
  validateRegistration(input: RegisterTemplateInput): void {
    ...
    if (!input.pdfUrl?.trim()) errors.push("Template PDF is required");
  ...
  validateCoordinates(
    ...
    const errors: string[] = [];
    if (pageNumber < 1) errors.push("pageNumber must be >= 1");
    if (xCoordinate < 0 || yCoordinate < 0) errors.push("Coordinates must be >= 0");
    if (xCoordinate > MAX_PAGE_DIMENSION || yCoordinate > MAX_PAGE_DIMENSION) {
      errors.push("Coordinates exceed page bounds");
    }
    if (width != null && width < 0) errors.push("width must be >= 0");
    if (height != null && height < 0) errors.push("height must be >= 0");
    if (errors.length) throw new Error(errors.join("; "));
  }

  validateMappingInput(mapping: TemplateFieldMappingInput): void {
    if (!mapping.systemFieldId?.trim()) throw new Error("systemFieldId is required");
    if (!mapping.fieldKey?.trim()) throw new Error("fieldKey is required");
  ...
  async validateTemplateRecord(
    ...
    if (!template.pdfUrl?.trim()) errors.push("Template must contain a PDF");
