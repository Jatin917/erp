...
export class ReportEngine {
  async run(request: ReportRunRequest): Promise<ReportRunResponse> {
    ...
    const registryByKey = new Map(
      registryFields.map((f) => [f.fieldKey, f])
    );
    ...
    const studentFields = studentFieldKeys
      ...
      .filter((f): f is FieldRegistry => Boolean(f));

    const { enrollmentIds, rows: scopeRows } =
      await scopeProvider.resolveScope(
        {
    ...
    const providerResults: ProviderFetchResult[] = [scopeRows];

    const parallelKeys: ProviderKey[] = ["attendance", "fees", "custom"];

    const parallelTasks = parallelKeys
      .filter((key) => grouped[key]?.length)
      .map(async (key) => {
        const provider = providerRegistry.get(key);
        if (!provider) return {} as ProviderFetchResult;
        ...
        if (key === "student") return {} as ProviderFetchResult;
    ...
    const mergedRows = aggregatorService.merge(
      enrollmentIds,
      fieldKeys,
      providerResults
    );
  ...
  private async resolveCurrentSessionId(branchId: string): Promise<string> {
