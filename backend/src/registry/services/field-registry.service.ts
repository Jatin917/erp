...
export const getActiveRegistryFields = async (filters?: { sourceModule?: string; groupKey?: string }) => {
  const cache = getFieldRegistryCache();
  if (cache.size === 0) await loadFieldRegistryCache();

  let rows = [...getFieldRegistryCache().values()];
  if (filters?.sourceModule) rows = rows.filter((row) => row.sourceModule === filters.sourceModule);
  if (filters?.groupKey) rows = rows.filter((row) => row.groupKey === filters.groupKey);
  return rows;
};
