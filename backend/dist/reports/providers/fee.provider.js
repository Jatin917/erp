...
export class FeeProvider {
    ...
    async fillPendingFees(enrollmentIds, rows) {
        ...
        for (const row of docs) {
            if (!rows[row.enrollmentId])
                rows[row.enrollmentId] = {};
    ...
    async fillTotalFeesPaid(enrollmentIds, rows) {
    ...
    async fillLatestFeeDocRaw(enrollmentIds, fields, rows) {
        ...
        for (const enrollmentId of enrollmentIds) {
            ...
            if (!rows[enrollmentId])
                rows[enrollmentId] = {};
            for (const field of fields) {
                if (field.sourceTable !== "FeeDoc" || !field.sourceColumn)
                    continue;
                const value = doc[field.sourceColumn];
...
//# sourceMappingURL=fee.provider.js.map